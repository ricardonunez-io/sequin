...
- TODO: Do we call it a table stream or a stream?
- TODO: Do we show example config files?
- TODO: Document where we lay out trade-offs of using webhooks

*/}

{/* Features we need before launching this guide:

- Message grouping where we detect when the stream/table is an event table, and auto-specify the group columns
- Long-poll endpoint

- Don't NEED, but you want to be able to batch webhooks
- Don't NEED, but you want a place in the console to manage API keys

*/}

{/* Other features that would make audit logging amazing:

- filters that let you compare `new` and `old` values of fields

*/}

Audit logs are an essential toolfor tracking and recording changes in your database. With Sequin, you can create comprehensive audit logs that help you:
...
{/* TODO: Assumes you already have a database connected */}

{/* Talks about dev --> prod */}
...
If you only care about replicating the latest version of each row from one table to another, the guide "[Sync a table from one database to another](/guides/sync-table)" might be more appropriate.

As you'll see, the destination table can be in the same database as the source table, or a different database.

If your needs are simple, you'll just need to create a [Change Capture Pipeline](#create-a-change-capture-pipeline) to stream your table's WAL to an event table in your database. You can optionally [create a view](#create-a-view) to shape the schema into a format convenient for your use case:

{/* TODO: diagram */}

However, if you need to transform the data in some way before writing it to your audit table, you can create a [Stream](/streams) to consume the changes from the Change Capture Pipeline's event table. Then, you can perform any transformations you need, before writing the data to your audit table:

{/* TODO: diagram */}

## Create a Change Capture Pipeline

Postgres' WAL is ephemeral and hard to query, so not a suitable place to store changes. Instead, you can use Sequin's [Change Capture Pipeline](/capture-changes) to stream your table's WAL to an event table in your database.

You'll create a Change Capture Pipeline for each table you want to create an audit log for. You can have multiple pipelines write to the same `sequin_events` table. Then, later, you can create a stream of that table to ingest the changes into your audit table.

### Create your first Change Capture Pipeline

In the Sequin web console, click on the **Capture Changes** tab. Click "Create Pipeline".
...
<Frame>
  <img style={{maxWidth: '500px'}} src="/images/guides/audit-logging/source-config.png" alt="Source configuration" />
</Frame>
...
<Frame>
  <img style={{maxWidth: '500px'}} src="/images/guides/audit-logging/create-event-table-modal.png" alt="Create event table modal" />
</Frame>

One important decision you need to make is what **retention policy** to use for the `sequin_events` table. The retention policy dictates how long Postgres should keep event data in the `sequin_events` table.
...
### Create Change Capture Pipelines for other tables
...
### Confirm your Change Capture Pipelines are working
...
For example, maybe you want to make a view called `subscription_logs` that contains only the logs for the `subscriptions` table. And maybe you want to pull a few fields out of the `record` JSONB column and into a top-level column in the view, like `subscription_id` and `status`:
...
Instead, see how far you can get with views and indexes. If you start to hit limits there, you may have reached a point where you should stream the `sequin_events` table to your workers to ingest the changes into another audit table with a schema you fully control.

<Check>
   If the `sequin_events` table is sufficient for your audit log needs, you can stop here. Otherwise, continue on to [Create a stream](#create-a-stream) so you can consume the changes into your audit table.
</Check>
...
Navigate to the **Streams** tab in the Sequin web console. Click **Create Stream**.
...
<Info>
  If you created your Change Capture Pipelines for a dev/staging database, you can create your consumer groups in the same database. Later, we'll show you how to [move them to production](#deploying-to-production).
</Info>

Sequin is capturing changes to your tables into the `sequin_events` table. You can setup a worker to process these changes, transforming them as needed before [upserting them into your audit table](#upserting-into-your-audit-table).

This section will show you how to consume the rows in the `sequin_events` table with a consumer group via the [Consumer Group API](/consume-api/overview). You can use one of Sequin's SDKs or the API directly to pull changes from your stream.
...
Under **Source**, select the `sequin_events` table stream.
...
<Frame>
  <img style={{maxWidth: '500px'}} src="/images/guides/audit-logging/records-to-process.png" alt="List of available filters to apply to the changes you want to capture" />
</Frame>
...
<Info>
  Sequin will have detected that this is an event table and selected the appropriate group columns, `source_table_oid`, `record_pks`, and `seq`.

  This means that events corresponding to the same row will be inserted in order into your audit table. This is important, as it will prevent race conditions (where there are "holes" in the history of changes for a given row, because two workers are processing the same row's changes simultaneously).
</Info>

Under **Consumer group configuration**, you can tweak settings like the **visibility timeout** as desired. The visibility timeout dictates how long workers/consumers in your consumer group should be given to process their batch of messages. If they don't finish processing within the visibility timeout, the messages will be made available to other consumers in the group.
...
func processEvents(ctx context.Context, msgs []sequin.Message) error {
    // SQL for upserting events into the audit log
    sql := `
    ...
    // Create a batch for efficient upserting
    batch := &pgx.Batch{}
    for _, event := range msgs {
        // Parse the new record JSON
        ...
        // Handle old values if they exist
...
See "[Upserting into your audit table](#upserting-into-your-audit-table)" for the implementation details of `upsert_events`.
...
{/* {{TODO: Introduce batching for webhooks}} */}

Instead of pulling events from a consumer group, you can have Sequin push events to a webhook endpoint. Each webhook payload will contain one event. Events are processed exactly once per subscription.
...
Under **Records to process**, optionally add a filter. For example, if you want this webhook to only receive events for the `subscriptions` table, you can add a filter on `table_name = 'subscriptions'`.
...
Your webhook endpoint will receive POST requests with event payloads. The payload will contain the same fields as the messages from the consumer group API.
...
app.post('/webhooks/audit-events', async (req, res) => {
  try {
    // The webhook payload contains a record and metadata
    const { record, metadata } = req.body;
    
    // Upsert into your audit table
    await db.query(`
      insert into user_permissions_log (
        event_id, user_id, permission, action, old_values, new_values
      )
      ...
      on conflict (event_id) do update set
        ...
        new_values = excluded.new_values
    `, [
      metadata.consumer.id, // Use the consumer ID as the event ID
      record.user_id,
      record.permission,
      metadata.action,
      null, // No old_values in this payload shape
      record
    ]);
...
If your endpoint returns a non-200 status code or fails to respond within the configured timeout, Sequin will retry the webhook delivery according to your retry policy.

### Monitor webhook deliveries

You can monitor webhook status on the Sequin dashboard, including:
- Failed deliveries and their error messages
- Outstanding deliveries waiting to be retried
- Webhook latency and success rates

This helps you identify and debug any issues with your webhook processing.
...
### Mapping events to your audit table

You'll flatten events from a nested JSON structure into a flat structure that matches your audit table.

Sequin sends events to your consumer group in JSON. JSON's types are not as rich as Postgres' types. So you may need to cast the types appropriately.
...
{/* TODO: We'll talk about exporting config, tweaking for prod */}
