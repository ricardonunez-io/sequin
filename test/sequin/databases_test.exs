defmodule Sequin.DatabasesTest do
  ...
  describe "Sequence CRUD operations" do
    ...
    test "get_sequence_for_account/2 retrieves a sequence", %{account: account, database: database} do
      ...
      assert {:ok, retrieved_sequence} = Databases.get_sequence_for_account(account.id, id: sequence.id)
    ...
    test "get_sequence_for_account/2 returns error for non-existent sequence", %{account: account} do
      assert {:error, _} = Databases.get_sequence_for_account(account.id, id: Ecto.UUID.generate())
    end
    ...
    test "delete_sequence/1 deletes a sequence", %{database: database} do
      ...
      assert {:error, _} = Databases.get_sequence_for_account(database.account_id, id: sequence.id)
    end
