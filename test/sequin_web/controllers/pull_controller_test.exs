defmodule SequinWeb.PullControllerTest do
  ...
  describe "receive, batch size behavior with wait" do
    test "returns immediately when full batch is available", %{conn: conn, consumer: consumer} do
      ...
      assert_elapsed_under(100, fn ->
        conn = get(conn, ~p"/api/http_pull_consumers/#{consumer.id}/receive", batch_size: 3, wait_for: 5000)
        assert %{"data" => messages} = json_response(conn, 200)
        assert length(messages) == 3
      end)
    end

    test "returns partial batch when timeout reached", %{conn: conn, consumer: consumer} do
      # Insert just 1 message when batch_size is 3
      ConsumersFactory.insert_deliverable_consumer_record!(consumer_id: consumer.id, source_record: :character)

      assert_elapsed_at_least(100, fn ->
        conn = get(conn, ~p"/api/http_pull_consumers/#{consumer.id}/receive", batch_size: 3, wait_for: 101)
        assert %{"data" => messages} = json_response(conn, 200)
        assert length(messages) == 1
      end)
    end

    test "returns as soon as batch size is reached during wait", %{conn: conn, consumer: consumer} do
      Task.Supervisor.async_nolink(Sequin.TaskSupervisor, fn ->
        for _ <- 1..3 do
          Process.sleep(10)
          ConsumersFactory.insert_deliverable_consumer_record!(consumer_id: consumer.id, source_record: :character)
  ...
  describe "receive, wait_for, and batch_size parameter validation" do
    ...
    test "rejects invalid batch_size values", %{conn: conn, consumer: consumer} do
